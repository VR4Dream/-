...
import com.weijie.vr4dream.model.Cityinfo;
import com.weijie.vr4dream.utils.CitycodeUtil;
...
//        String area_str = FileUtil.readAssets(context, "distpicker.data.json");
//        province_list = parser.getJSONParserResult(area_str, "86");
//        city_map = parser.getJSONParserResultArray(area_str, province_list);
//        couny_map = parser.getJSONParserResultArray(area_str, city_map);
        String area_str = FileUtil.readAssets(context, "distpicker.data.json");
    ...
    public static class JSONParser {
        ...
        public HashMap<String, List<Cityinfo>> getJSONParserResultArray(
                String JSONString, List<Cityinfo> cityinfos) {
            HashMap<String, List<Cityinfo>> hashMap = new HashMap<String, List<Cityinfo>>();
            for (Cityinfo cityinfo : cityinfos) {
                JsonObject result = new JsonParser().parse(JSONString)
                        .getAsJsonObject().getAsJsonObject(cityinfo.getId());
                ...
                if(result!=null) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        info.setCity_name(entry.getValue().getAsString());
                        info.setId(entry.getKey());
                        infos.add(info);
                    }
                    hashMap.put(cityinfo.getId(), infos);
                }
        ...
        public HashMap<String, List<Cityinfo>> getJSONParserResultArray(
                String JSONString, HashMap<String, List<Cityinfo>> map) {
            HashMap<String, List<Cityinfo>> hashMap = new HashMap<String, List<Cityinfo>>();

            Iterator entries = map.entrySet().iterator();
            while (entries.hasNext()) {
                Map.Entry entry = (Map.Entry) entries.next();
                List<Cityinfo> list = (List<Cityinfo>)entry.getValue();
                for(Cityinfo cityinfo:list) {
                    ...
                    if(result!=null) {
                        ...
                        Iterator<?> iterator = result.entrySet().iterator();
                        while (iterator.hasNext()) {
                            Map.Entry<String, JsonElement> mapEntry = (Map.Entry<String, JsonElement>) iterator
                                    .next();
                            Cityinfo info = new Cityinfo();
                            info.setCity_name(mapEntry.getValue().getAsString());
                            info.setId(mapEntry.getKey());
                            infos.add(info);
                        }
                        hashMap.put(cityinfo.getId(), infos);
                    }
