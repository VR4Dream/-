...
public class ProgressWheel extends View {
    ...
    public ProgressWheel(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.progress_wheel));
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius = (int) a.getDimension(R.styleable.progress_wheel_mat_circle_radius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.progress_wheel_mat_fill_radius, false);

        barWidth = (int) a.getDimension(R.styleable.progress_wheel_mat_bar_width, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.progress_wheel_mat_rim_width, rimWidth);

        float baseSpinSpeed = a.getFloat(R.styleable.progress_wheel_mat_spin_speed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime = a.getInt(R.styleable.progress_wheel_mat_bar_spin_cycle_time, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.progress_wheel_mat_bar_color, barColor);

        rimColor = a.getColor(R.styleable.progress_wheel_mat_rim_color, rimColor);

        linearProgress = a.getBoolean(R.styleable.progress_wheel_mat_linear_progress, false);

        if (a.getBoolean(R.styleable.progress_wheel_mat_progress_indeterminate, false)) {
            spin();
        }
